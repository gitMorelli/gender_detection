...
def main(args):
    ...
    for file in input_file_name:
        ...
        source_data=s_d #they are equal since the models are applied to the same preprocessed data
        model_used.append(row['model'].values[0])
        row=df_log[df_log['experiment']==s_d]
        extracted_from=row['type'].values[0] # same as above
    #args for training
    ...
    train_1 = pd.read_csv(input_file_1).drop(columns=['file_name'], errors='ignore')
    train_2 = pd.read_csv(input_file_2).drop(columns=['file_name'], errors='ignore')
    if is_kaggle:
